...
- 必须确保其提交的独立访客（UV）数据全部为真人流量，不能包含爬虫或机器人访问，如来自于百度统计、谷歌统计、自建基于js的统计方案
- 用户提供Google Analytics等前端分析工具的访问报告，注明需包含独立访客数、会话数、跳出率等关键指标或采用真实用户监控（RUM）方案
- 如有条件，可要求用户开放GA只读权限，或上传导出的PDF/CSV报告，避免数据造假
- Cloudflare 等 CDN平台提供的访问图，无效
- 域名成立时间需要在半年以上，免费二级域名网站根据质量限定门槛，纯数字xyz/best域名可能会降级处理
- 要求在面板上验证通过后，先在申请资助的网站上添加赞助说明[iframe组件](https://support.nodeget.com/page/promotion?id={your_id})，以降低审核成本
